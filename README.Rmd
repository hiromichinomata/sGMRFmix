...
You should set `K` a large enough number because the algorithm identifies major dependency patterns from the data via the sparse mixture model.

You should determine `rho` an optimal value to maximize the performance of anomaly detection.
...
The package provides the `sGMRFmix()` function to fit the model named *Sparse Gaussian Markov Random Field Mixtures* (Ide et al., 2016).
It can automatically handle multiple operational modes.
And it can also compute variable-wise anomaly scores.
...
As the result of we set `K` to 7 and fit the model, the number of mixtures `K-est` has been sparsely estimated 2.
The weights of the mixtures are displayed as `pi`. 
They are near 0.5.
...
To tell it, you have no choice but to tune the hyperparameter `rho`.
...
Optimal `rho` value that has the best performance to detect anomalies we think is 1.29.
...
In the above, the model has identified that synthetic data consists of two operational modes.
...
We should check estimated structures and decide whether it is reasonable.
